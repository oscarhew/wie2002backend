...
foreach($subCategoryArr as $singleCategory){
    ...
    $sql = "SELECT course.id, course.title, course.description, course.image, author.name, sub_category.id, sub_category.name
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $data[] = $row;
            $subCategoryName = $row['course.title'];
            $subCategoryName = $row['auhtor.name'];
        }
