...
foreach($subCategoryArr as $singleCategory){
    $subCategoryName;
    $sql = "SELECT course.course_id, course.course_name, course.image_url, course.course_description, course.instructor_id, sub_category.id, sub_category.name
    FROM course". " INNER JOIN sub_category ON sub_category.id = course.category_id " ." WHERE sub_category.category_id=". $singleCategory['id'];
